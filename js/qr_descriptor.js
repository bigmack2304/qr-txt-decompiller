...
const qr_name_n = document.getElementsByClassName("file_info__name")[0].children[0];           // инфа об имяи фаила
...
file_wath[0].style.display = 'none';
...
function file_reader_txt(file) {                                // читаем txt фаил
    ...
    reader.onload = () => {
        ...
        file_wath[0].style.display = 'block';                      // включить видимость для окна просмотра документа
...
function file_reader_csv(file) {                                    // читаем csv фаил
    ...
    const load_comlete = () => {                                    // по завершению чтения фаила
        if (!CustomErrEditor.get_error_info("read_file")) {
            file_txt = file_txt.split("\n");                          // преобразуем в массив, строка - ячейка
