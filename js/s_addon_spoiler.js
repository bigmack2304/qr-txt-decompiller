...
if (is_object_valid(addon_spoilers)) {
  for(let i=0; i < addon_spoilers.length; i++) {                     
	addon_spoilers[i].addEventListener('click', function (evt) {                // повесить на них обработчик "клик мышкой"
		addon_spoiler_on_click(evt);
	}); 

	addon_spoilers[i].addEventListener('keydown', function(evt) {               // повесить на них обработчик "выбор enter,ом"
